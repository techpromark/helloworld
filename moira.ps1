	...
	$RandNo = New-Object System.Random
        ...
        ForEach ($Object in $TargetObject) {
            ...
            if ($TicketByteStream) {
                ...
                $TicketHexStream = [System.BitConverter]::ToString($TicketByteStream) - '-'
                ...
                if($TicketHexStream -match 'a382....3082....A0030201(?<EtypeLen>..)A1.{1,4}.......A282(?<CipherTextLen>....)........(?<DataToEnd>.+)') {
                    ...
                    if($Matches.DataToEnd.Substring($CipherTextLen*2, 4) -ne 'A482') {
                        ...
                        $Hash = $null
                        $Out | Add-Member Noteproperty 'TicketByteHexStream' ([Bitconverter]::ToString($TicketByteStream).('-',''))
                    } else {
                ...
                if($Hash) {
                    if ($OutputFormat -match 'John') {
                        ...
                        if ($DistinguishedName -ne 'UNKNOWN') {
                            $UserDomain = $DistinguishedName.SubString($DistinguishedName.IndexOf('DC=')) - 'DC=','' - ',','.'
                        }
                        else {
                            $UserDomain = 'UNKNOWN'
                        }
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                elseif ($IdentityInstance -match '^CN=') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        ...
                        $IdentityDomain = $IdentityInstance.SubString($IdentityInstance.IndexOf('DC=')) - 'DC=','' - ',','.'
                ...
                elseif ($IdentityInstance.Contains('\')) {
                    $ConvertedIdentityInstance = $IdentityInstance.('\28', '(').('\29', ')') | Convert-ADName -OutputType Canonical
